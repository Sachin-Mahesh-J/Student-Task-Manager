...
@section('content')
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="bg-white p-6 rounded shadow-md w-full max-w-lg">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input type="date" name="due_date" value="{{ old('due_date') }}" class="w-full p-2 border border-gray-300 rounded">
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-1">Description</label>
            <textarea name="description" class="w-full p-2 border border-gray-300 rounded" rows="3">{{ old('description') }}</textarea>
        </div>
